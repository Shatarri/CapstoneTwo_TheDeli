...
public class DeliCiousApp {
    ...
    private static Order.Drink createDrink(Scanner scanner) {
        ...
        String size = scanner.nextLine();
        System.out.println("Enter drink flavor ( Cola, Lemonade): ");
        String flavor = scanner.nextLine();

        return new Order.Drink(size, flavor, 2.50); // Set a default price for the drink
    }

    private static Order.Chips createChips(Scanner scanner) {
        System.out.println("Enter chips flavor (Original, BBQ): ");
        String flavor = scanner.nextLine();

        return new Order.Chips(flavor, 1.50);
